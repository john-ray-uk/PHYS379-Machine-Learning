...
class Layer:
    ...
    def __init__(self, nInputs, nNeurons):
        ...
        '''
        # Connections = weights
        self.weights1 = np.random.randn(nInputs, nNeurons) * 0.01
        self.weights2 = np.random.randn(nInputs, nNeurons) * 0.01
        # Neurons = biases
    ...
    def forward(self, inputs):
        ...
        """
        self.inputs = inputs
        self.output = np.dot(inputs, self.weights1) + self.biases
        
    def forwardpoly(self, inputs):
        
        self.inputs = inputs
        self.output = np.dot((inputs**2), self.weights1) + np.dot(inputs, self.weights2) + self.biases
    
    def forwardlog(self, inputs):
        self.inputs = inputs
        non_zero_constant = 1e-6
        log_inputs = np.log(np.abs(inputs) + non_zero_constant)
        self.output = np.dot(log_inputs, self.weights1) + self.biases
#----------------------------------------------------------
...
'''
layer1.forwardlog(inputs)
activation1.forward(layer1.output)
layer2.forwardpoly(activation1.output)
